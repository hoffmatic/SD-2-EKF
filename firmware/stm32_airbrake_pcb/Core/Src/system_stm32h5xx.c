 ...
 *
 * What this file does:
 *   This ST system file sets up basic STM32H5 system values before main.c runs. It defines clock constants, vector-table settings, and SystemInit behavior.
 *
 * Process flow:
 *   Reset code calls SystemInit before main. HAL and CMSIS functions then use SystemCoreClock to understand CPU timing.
 *
 * Main variables and what can be changed:
 *   HSE, HSI, CSI clock constants and vector-table offset are the main settings, but they should only change when the clock tree or boot layout changes.
 *
 * Assumptions:
 *   This file matches the STM32H562 target and CubeMX clock setup.
 *
 * What is missing:
 *   No project-specific fault logging or board-revision handling lives here.
 */
